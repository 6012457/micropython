...
pid_info_label = tk.Label(root, width=50, height=3, text='PID调参小工具', font=('Arial', 15))
...
# KP 滑动条
kp_scale = tk.Scale(right_frame, label='KP', from_=0, to=5, orient=tk.HORIZONTAL,
             length=500, showvalue=0, tickinterval=3, resolution=0.001,command=update_kp)
kp_scale.pack()
# KI 滑动条
ki_scale = tk.Scale(right_frame, label='KI', from_=0, to=10, orient=tk.HORIZONTAL,
             length=500, showvalue=0, tickinterval=1, resolution=0.01, command=update_ki)
ki_scale.pack()
# KD 滑动条
kd_scale = tk.Scale(right_frame, label='KD', from_=0, to=1, orient=tk.HORIZONTAL,
             length=500, showvalue=0, tickinterval=2, resolution=0.001, command=update_kd)
kd_scale.pack()
...
def timer_callback():
    ...
    axes.set_xticks([])
    axes.set_ylim(-10 + target, target+10)
    axes.set_yticks([-10 + target, target, target+10])
